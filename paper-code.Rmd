...
pacman::p_load(tidyverse, tidymodels, dplyr, readr, sf, spdep, tmap, readxl, here, GWmodel, RColorBrewer)
```
...
map_pc1 <- tm_shape(na.omit(london_polygon)) + 
  tm_polygons("PC1", 
              fill.scale = tm_scale_continuous(values = "-brewer.rd_bu", limits = c(pc_lower, pc_upper), ticks=c(-4,-2,0,2,4,6,8)),
              col_alpha = 0, 
...
map_income <- tm_shape(na.omit(london_polygon)) + tm_polygons("household_median_income_thousands", tm_scale_continuous(values="brewer.purples",limits=c(10,95)), col_alpha = 0, tm_legend(title="", frame=FALSE)) + tm_layout(frame=FALSE, legend.text.size=1.2, legend.reverse = TRUE)

# plot BAME
map_BAME <- tm_shape(na.omit(london_polygon)) + tm_polygons("BAME_2011_perc", tm_scale_continuous(values="brewer.purples"), col_alpha = 0, tm_legend(title="", frame=FALSE)) + tm_layout(frame=FALSE, legend.text.size=1.2, legend.reverse=TRUE)

# plot walk-time
map_foodwalk <- tm_shape(na.omit(london_polygon)) + tm_polygons("food_walk", tm_scale_continuous(values="brewer.purples",limits = c(0, 100)), col_alpha = 0, tm_legend(title="", frame=FALSE)) + tm_layout(frame=FALSE, legend.text.size=1.2, legend.reverse=TRUE)

# plot car ownership
map_carownership <- tm_shape(na.omit(london_polygon)) + tm_polygons("carownership_perc", tm_scale_continuous(values="brewer.purples",limits=c(10,98)), col_alpha = 0, tm_legend(title="", frame=FALSE)) + tm_layout(frame=FALSE, legend.text.size=1.2, legend.reverse=TRUE)

# plot age
map_age <- tm_shape(na.omit(london_polygon)) + tm_polygons("avg_age", tm_scale_continuous(values="brewer.purples"), col_alpha = 0, tm_legend(title="", frame=FALSE)) + tm_layout(frame=FALSE, legend.text.size=1.2, legend.reverse = TRUE)
...
LISA_map <- tm_shape(clustering_data) +
  tm_polygons("clusters_HH_LL",
              fill.scale = tm_scale_categorical(
                values = c("red", "blue"),
                labels = c("Hot Spot", "Cold Spot")
              ),
              col_alpha = 0,
              fill.legend = tm_legend(title = "", frame = FALSE, na.show = FALSE, title.size = 1.2, text.size = 1, position=c("right","top"))) +
  tm_borders(lwd = 0.1) +
...
tm_shape(gwr_data)+tm_polygons(fill="gwr_income", fill.scale = tm_scale_continuous(values="-brewer.prgn", limits=c(lower,upper), ticks=c(-4,-3,-2,-1,0,1,2,3)), col_alpha = 0, tm_legend(title="", frame=FALSE)) + tm_shape(gwr_data %>% filter(abs(gwr_income_t)>1.96 & clusters == "High-High")) + tm_borders(col="black", lwd=0.25) + 
  tm_layout(frame=FALSE, legend.text.size = 1, legend.reverse=TRUE) + tm_title("")

tm_shape(gwr_data)+tm_polygons(fill="gwr_foodwalk", fill.scale = tm_scale_continuous(values="-brewer.prgn", limits=c(lower,upper), ticks=c(-4,-3,-2,-1,0,1,2,3)), col_alpha = 0, tm_legend(title="", frame=FALSE)) + tm_shape(gwr_data %>% filter(abs(gwr_foodwalk_t)>1.96 & clusters == "High-High")) + tm_borders(col="black", lwd=0.25) + tm_layout(frame=FALSE,legend.text.size = 1, legend.reverse=TRUE) + tm_title("")
...
## Figure 7 - facet
```{r}
# combine maps into single figure 
#recreate plots here with common layout applied
...
# Choropleth maps (left column)
map_income <- tm_shape(na.omit(london_polygon)) +
  tm_polygons("household_median_income_thousands", palette = "brewer.purples", col_alpha = 0,
              legend.title = "", legend.frame = FALSE)

map_BAME <- tm_shape(na.omit(london_polygon)) +
  tm_polygons("BAME_2011_perc", palette = "Purples", col_alpha = 0,
              legend.title = "", legend.frame = FALSE)

map_carownership <- tm_shape(na.omit(london_polygon)) +
  tm_polygons("carownership_perc", palette = "Purples", col_alpha = 0,
              legend.title = "", legend.frame = FALSE) +
  common_layout

map_age <- tm_shape(na.omit(london_polygon)) +
  tm_polygons("avg_age", palette = "Purples", col_alpha = 0,
              legend.title = "", legend.frame = FALSE) +
  common_layout


# GWR maps (right column)
gwr_income_map <- tm_shape(gwr_data) +
  tm_polygons("gwr_income", palette = "-PRGn", col_alpha = 0,
              legend.title = "", legend.frame = FALSE,
              style = "cont") +
  tm_shape(gwr_data %>% filter(abs(gwr_income_t) > 1.96 & clusters == "High-High")) +
  tm_borders(col = "black", lwd = 0.25) +
  common_layout

gwr_BAME_map <- tm_shape(gwr_data) +
  tm_polygons("gwr_BAME", palette = "-PRGn", col_alpha = 0,
              legend.title = "", legend.frame = FALSE,
              style = "cont") +
  tm_shape(gwr_data %>% filter(abs(gwr_BAME_t) > 1.96 & clusters == "High-High")) +
  tm_borders(col = "black", lwd = 0.25) +
  common_layout

gwr_carownership_map <- tm_shape(gwr_data) +
  tm_polygons("gwr_carownership", palette = "-PRGn", col_alpha = 0,
              legend.title = "", legend.frame = FALSE,
              style = "cont") +
  tm_shape(gwr_data %>% filter(abs(gwr_carownership_t) > 1.96 & clusters == "High-High")) +
  tm_borders(col = "black", lwd = 0.25) +
  common_layout

gwr_age_map <- tm_shape(gwr_data) +
  tm_polygons("gwr_age", palette = "-PRGn", col_alpha = 0,
              legend.title = "", legend.frame = FALSE,
              style = "cont") +
  tm_shape(gwr_data %>% filter(abs(gwr_age_t) > 1.96 & clusters == "High-High")) +
  tm_borders(col = "black", lwd = 0.25) +
  common_layout

# Arrange 8 maps in 2 columns × 4 rows
#...
...
tm_shape(gwr_data) + tm_polygons("gwr_localr2", fill.scale=tm_scale_continuous(values="carto.teal", limits=c(min_r2,max_r2), ticks=c(0.2,0.4,0.6,0.8)), col_alpha=0, tm_legend(title="", frame=FALSE)) + tm_shape(gwr_data %>% filter(clusters == "High-High")) + tm_borders(col="white", lwd=0.5) + tm_layout(frame=FALSE, legend.text.size=1, legend.reverse=TRUE) + tm_title("") 
```
...
# Define palette and label mappings
pal_colours <- brewer.pal(5, "Set2")
value_order <- c("C.avg_age", "C.BAME_2011_perc", "C.carownership", "C.food_walk", "C.household_median_income")
label_order <- c("Age", "BAME", "Car ownership", "Walk time", "Income")

tm_shape(contribution_data_long %>% filter(alpha_fd == 0)) +
  tm_polygons("most_important",
              fill.scale = tm_scale_categorical(
                values = setNames(pal_colours, value_order),
...
tm_shape(contribution_data_long %>% filter(alpha_fd == 1)) +
  tm_polygons("most_important",
              fill.scale = tm_scale_categorical(
                values = setNames(pal_colours, value_order),
                labels = setNames(label_order, value_order),
                label.na = "Insignificant"
              ),
...
tm_layout(frame = FALSE, legend.frame = FALSE, legend.text.size=1.2)
```
...
tm_shape(contribution_data_long %>% filter(alpha_fo == 0)) +
  tm_polygons("most_important_fo",
              fill.scale = tm_scale_categorical(
                values = setNames(pal_colours, value_order),
                label.na = "Insignificant"
              ),
...
tm_shape(contribution_data_long %>% filter(alpha_fo == 1)) +
  tm_polygons("most_important_fo",
              fill.scale = tm_scale_categorical(
                values = setNames(pal_colours, value_order),
                labels = setNames(label_order, value_order),
...
tm_shape(gwr_data_sf) + 
  tm_fill("category", palette = c("centre"="black", "nearest"="orange", "outer"="white")) + tm_layout(frame = FALSE, legend.show = FALSE) + tm_borders("black", lwd = 0.2)
```
